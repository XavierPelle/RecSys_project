...
let stream = fs.createReadStream("train_sessions.csv");
let csvData = [];
let batchSize = 5000; 
  ...
  .on("end", function() {
    ...
    pool.connect((err, client, done) => {
      ...
      client.query(createTableQuery, (err, res) => {
        ...
        const query = "INSERT INTO train_sessions (session_id, item_id) VALUES ($1, $2)";
        ...
        csvData.forEach((row, index) => {
          ...
          if (batch.length === batchSize || index === csvData.length - 1) {
            client.query(
              "INSERT INTO train_sessions (session_id, item_id) VALUES " +
